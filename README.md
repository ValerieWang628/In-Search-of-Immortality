...
As the [dependency chart](https://github.com/ValerieWang628/In-Search-of-Immortality/blob/master/Game%20Dependency%20Flow%20Chart%20for%20In%20Search%20of%20Immortality.pdf) actually serves as a workflow logic map, it also includes the conditions, locations and attributes of each object. For example, an object, the willow trees, will be transformed into willow twigs if the player uses an ax to cut it. Then, the twigs will become a durable net in the inventory if the player weaves the twigs. For another instance, When wood sticks are constantly drilled by the player, a new object, fire, is created. However, at that stage, that fire has an attribute called temporary, meaning that it is a fire that can not last long. Thus, this fire cannnot be put into the player's inventory, and the player will not manage to use temporary fire to look for the mirror hidden in the grass because the fire might go out anytime. Similarly, a torch in this game cannot hold long-lasting fire unless it is dipped into the oil. Once it is dipped into the oil, that object will have a new attribute called 'oil-penetrated'. There are many subtle designs like this, and the dependency chart really helped a lot.
